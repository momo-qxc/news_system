let newsinfoobj = {
    ...
    submitComment: function () {
        ...
        $.post(service_path + "/news/comment/save", {
            phone: phone,
            nid: nid,
            content: content,
            createdate: new Date().toLocaleString(),
    ...
    loadComments: function (nid) {
        ...
        $.get(service_path + "/news/comment/getbynid", { nid: nid, phone: phone }, function (data) {
            ...
            if (list && list.length > 0) {
                for (let i = 0; i < list.length; i++) {
                    let comment = list[i];
                    let statusTag = (comment.status === 0) ? ' <span style="color:#f90; font-size:12px;">(待审核)</span>' : '';

                    html += '<div class="comment-item" style="position: relative;">';
                    ...
                    html += '    <span class="comment-time">' + (comment.createdate || "") + '</span>';
